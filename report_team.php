<?php
ob_start();
//In page all ticket we get the  name of the team that we want to make for it report. When we take the name of the team from this page we go to the table to check the tickets that we have that are connected to this team
...
if ($team_id) {
    ...
    require_once './src/team.php'; // Assuming you have a Team class for fetching team details

    $ticket = new Ticket();
    $team = new Team();

    // Fetch team details
    $teamDetails = $team->find($team_id);
    
    // Fetch the tickets for this team
    $teamTickets = $ticket->getTicketsByTeam($team_id,$branchId);
    ...
    <body>
        ...
        <div class="report">
            <h2>All Tickets for Team: <?php echo htmlspecialchars($teamDetails->name); ?></h2>
            <table class="ticket-details">
                ...
                <tbody>
                <?php if (!empty($teamTickets)): ?>
                    <?php foreach ($teamTickets as $ticket): ?>
                        <tr>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="10">No tickets found for this team.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
...
document.addEventListener("DOMContentLoaded", function () {
    const generatePDF = () => {
        ...
        // Add a title
        doc.setFontSize(18);
        doc.text( "All Tickets for Team: <?php echo htmlspecialchars($teamDetails->name); ?>", 105, 20, { align: "center" });
        
        doc.setFontSize(12);
        doc.text("ID User: <?php echo htmlspecialchars($teamDetails->id); ?>", 105, 30, { align: "center" });
        ...
        doc.save("<?php echo htmlspecialchars($teamDetails->name); ?>");
    };
