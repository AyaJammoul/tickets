...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['excel_file'])) {
    ...
    $insertStmt = $db->prepare("INSERT INTO branch (name, created_at, updated_at) 
        VALUES (?, ?, ?)");
    
    $updateStmt = $db->prepare("UPDATE branch SET name = ?, updated_at = ?
        WHERE id = ?");
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
        foreach ($sheet->getRowIterator() as $rowIndex => $row) {
            ...
            $branchId = (string)$cells[0]; // Get the ID from the first cell
            $name = (string)$cells[1];
            $created_at = date('Y-m-d H:i:s');
            $updated_at = date('Y-m-d H:i:s');

            // Check if the branch already exists using the ID
            $checkStmt = $db->prepare("SELECT id FROM branch WHERE id = ?");
            $checkStmt->bind_param("s", $branchId);
            $checkStmt->execute();
            $checkStmt->store_result();

            if ($checkStmt->num_rows > 0) {
                // branch exists, update the record
                $updateStmt->bind_param("sss", $name, $updated_at, $branchId);
                if (!$updateStmt->execute()) {
                    echo "Error updating row $rowIndex: " . $updateStmt->error . "<br>";
                }
            } else {
                // New branch, insert the record
