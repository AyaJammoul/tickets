...
$tickets = TicketManager::findAll2($branchId);
...
// Map manager ID to manager name for easy lookup
...
$sql = "SELECT id, ticket_id, image_path FROM ticket_m_images";
$res = $db->query($sql);
$images = [];
    ...
    imagePaths.forEach((path, index) => {
        ...
        carousel.innerHTML += `
            <div class="carousel-item ${isActive}">
                <img src="/upload/manager/${path}" class="d-block w-100" alt="Ticket Image">
            </div>
        `;
    });
                    ...
                    <table class="table table-bordered table-sm" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    // Get manager name based on manager ID
                                    echo isset($managers[$ticket->manager_id]) 
                                        ? htmlspecialchars($managers[$ticket->manager_id]) 
                                        : 'N/A';
                                    ?>
                                </td>
                                <td>
                                    <?php
                                    // Get requester name based on requester ID
                                    echo isset($requesters[$ticket->requester_id]) 
                                        ? htmlspecialchars($requesters[$ticket->requester_id]) 
                                        : 'N/A';
                                    ?>
                                </td>
                                <td>
