<?php
ob_start();
//Here we make print report according to how many ticket the requester have
...
if ($requester_id) {
    ...
    $requesterDetails = $requester->find($requester_id);
    
    // Fetch the tickets for this requester
    $requesterTickets = $ticket->getTicketsByRequester($requester_id);
    ...
    <a href="./all-ticket-records" class="btn btn-light position-absolute top-0 start-0 m-3">
        ...
        <div class="report">
            <h2>All Tickets for Requester: <?php echo htmlspecialchars($requesterDetails->name); ?></h2>
            <table class="ticket-details">
                ...
                <tbody>
                <?php if (!empty($requesterTickets)): ?>
                    <?php foreach ($requesterTickets as $ticket): ?>
                        <tr>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="10">No tickets found for this requester.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
...
document.addEventListener("DOMContentLoaded", function () {
    const generatePDF = () => {
        ...
        // Add a title
        doc.setFontSize(18);
        doc.text( "All Tickets for Requester: <?php echo htmlspecialchars($requesterDetails->name); ?>", 105, 20, { align: "center" });
        
        doc.setFontSize(12);
        doc.text("ID User: <?php echo htmlspecialchars($requesterDetails->id); ?>", 105, 30, { align: "center" });
        ...
        doc.save("<?php echo htmlspecialchars($requesterDetails->name); ?>");
    };
