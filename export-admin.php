...
try {
    ...
    $result = $db->query("SELECT a.id, a.name, a.email, a.phone, a.password, 
                                b.name AS branch_name, a.role, a.created_at, a.updated_at, a.department, 
                                a.job_title, a.phone_extension, a.location, a.preferred_language 
                          FROM admin a
    ...
    $writer->openToBrowser("admins_export.xlsx");

    // Add headers to the first row
    $headers = [
        "ID", "Name", "Email", "Phone", "Password", 
        "Branch Name", "Role", "Created At", "Updated At", "Department", 
        "Job Title", "Phone Extension", "Location", "Preferred Language"
    ];
    ...
    while ($record = $result->fetch_assoc()) {
        $rowData = [
            ...
            $record['role'],
            $record['created_at'], 
            $record['updated_at'], // corrected field name
            $record['department'], 
            $record['job_title'], 
