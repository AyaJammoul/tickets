...
require_once './src/ticket-u-image.php'; // Make sure to include the TicketImage class
...
if (!isset($_GET['id'])) {
    ...
    $ticketId = $_GET['id'];
    $ticket = TicketUser::find($ticketId);
...
//Here in the update if he wants to add jmage, we have the path to save the image in the upload file. Surely with the extentions that we selected
            ...
            if (isset($_POST['upload'])) {
                ...
                $targetDir = "./upload/user/" . $ticketId . "/";  
                    ...
                    foreach ($_FILES['filesToUpload']['name'] as $index => $fileName) {
                        ...
                        if (in_array($fileType, ['jpg', 'png', 'jpeg', 'gif', 'pdf', 'doc', 'docx'])) {
                            if (move_uploaded_file($fileTmpName, $targetFile)) {
                                ...
                                ImageU::create($ticketId, $imagePath);  
                            } else {
...
if (isset($_GET['delete'])) {
    ...
    $sql = "SELECT image_path FROM ticket_u_images WHERE id = ?";
    $stmt = $db->prepare($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        $filePath = './upload/user/' . $image['image_path'];
        ...
        $deleteSql = "DELETE FROM ticket_u_images WHERE id = ?";
        $deleteStmt = $db->prepare($deleteSql);
        $deleteStmt->bind_param("i", $imageId);
        $deleteStmt->execute();

        $msg = "File deleted successfully.";
        $folderPath = './upload/user/' . $ticketId; // Path to the folder
...
if (is_dir($folderPath)) {
    ...
    if (count(scandir($folderPath)) == 2) { // '.' and '..' only
        ...
        $err = "File not found.";
            ...
            $files = ImageU::getImages($ticketId); 
        }
    }
...
<div id="content-wrapper">
    <div class="container-fluid">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Update Ticket user</li>
        </ol>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] . '?id=' . $ticketId; ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group row col-lg-8 offset-lg-2">
                        <label for="phone" class="col-sm-3 col-form-label">user Phone </label>
                        <div class="col-sm-9">
                                ...
                                <div class="card">
                                    <?php if (in_array(pathinfo($file['image_path'], PATHINFO_EXTENSION), ['pdf'])): ?>
                                        <div class="card-body text-center">
                                            <a href="./upload/user/<?php echo $file['image_path']; ?>" target="_blank" class="btn btn-primary">View PDF</a>
                                        </div>
                                    <?php elseif (in_array(pathinfo($file['image_path'], PATHINFO_EXTENSION), ['doc', 'docx'])): ?>
                                        <div class="card-body text-center">
                                            <a href="./upload/user/<?php echo $file['image_path']; ?>" target="_blank" class="btn btn-primary">View Word File</a>
                                        </div>
                                    <?php else: ?>
                                        <img src="./upload/user/<?php echo $file['image_path']; ?>" class="card-img-top" alt="File Image">
                                    <?php endif; ?>
                                    <div class="card-body text-center">
