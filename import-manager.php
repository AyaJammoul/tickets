...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['excel_file'])) {
    ...
    $insertStmt = $db->prepare("INSERT INTO manager (name, email, phone, password, role, created_at, updated_at, department, job_title, phone_extension, location, preferred_language, branch_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $updateStmt = $db->prepare("UPDATE manager SET name = ?, email = ?, phone = ?, password = ?, role = ?, updated_at = ?, department = ?, job_title = ?, phone_extension = ?, location = ?, preferred_language = ?, branch_id = ? WHERE id = ?");
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
        foreach ($sheet->getRowIterator() as $rowIndex => $row) {
            ...
            $checkStmt = $db->prepare("SELECT id FROM manager WHERE id = ?");
            $checkStmt->bind_param("s", $managerId);
            $checkStmt->execute();
            $checkStmt->store_result();

            if ($checkStmt->num_rows > 0) {
                // Update existing manager
                $updateStmt->bind_param("sssssssssssss", $name, $email, $phone, $password, $role, $updated_at, $department, $job_title, $phone_extension, $location, $preferred_language, $branchId, $managerId);
